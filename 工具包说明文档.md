...
> 参数与 `.claude/sdk/pwd.md` 保持一致即可。

3. **配置数据库连接**

编辑 `.claude/sdk/pwd.md`，填入泛微 OA 数据库信息：
...
> `db_config.py` 会自动从 `pwd.md` 读取配置，不需要改代码文件。
...
A：先检查 `.claude/sdk/pwd.md` 数据库配置是否正确；再检查 MCP Server 是否正常运行；最后对照 `modeling-sdk-reference.md` 确认参数格式。
